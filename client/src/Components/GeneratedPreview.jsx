...
const INITIAL_QUESTIONS = [
  {
    ...
    difficulty: "Medium", topic: "Light reactions",
  },
  {
    ...
    difficulty: "Easy", topic: "Light reactions",
  },
  {
    ...
    difficulty: "Hard", topic: "Calvin cycle",
  },
  {
    ...
    difficulty: "Easy", topic: "Chloroplast",
  },
  {
    ...
    difficulty: "Medium", topic: "Photolysis",
  },
  {
    id: 6,
...
const QUESTION_TYPES = ["MCQ", "Short Answer"];
...
const GeneratedPreview = ({ chapter = "Chapter 4 — Photosynthesis", onSave, onDiscard }) => {
  ...
  const [selected, setSelected] = useState(new Set([1, 2, 4, 5]));
  ...
  return (
    <div className="bg-bg min-h-screen p-8 font-sans">
      ...
      <div className="max-w-2xl mx-auto flex flex-wrap items-center gap-2 mb-4">
        ...
        <button onClick={() => onSave?.(questions.filter((q) => selected.has(q.id)))} className="px-3 py-1.5 rounded-lg border border-primary bg-primary text-white text-xs hover:bg-primary-hover transition-colors flex items-center gap-1.5">
          <SaveOutlined style={{ fontSize: 11 }} /> Save to bank
        </button>
      </div>
        ...
        {questions.map((q) => (
          ...
          >
              ...
              <div className="grid grid-cols-2 gap-1.5 pl-14 mb-3">
                {q.options.map((opt, i) => (
                  ...
                  >
                    ...
                    {opt}
                    {i === q.correct && <CheckOutlined style={{ fontSize: 9, marginLeft: "auto" }} />}
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="flex items-center gap-2 pl-14 flex-wrap">
              ...
              <span className="text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-[#DBEAFE] text-[#1E40AF]">
                {q.topic}
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    onChange={(e) =>
                      setDraftQuestion((prev) => {
                        ...
                        if (nextType === "Short Answer") {
                          return { ...prev, type: nextType, answer: prev.answer || "", options: ["", "", "", ""], correct: 0 };
                        }
                ...
                <div>
                  <label className="block text-sm font-medium text-text-secondary mb-1.5">
                    {draftQuestion.type === "Short Answer" ? "Expected answer" : "Correct option"}
                  </label>
      ...
      <div className="max-w-2xl mx-auto flex items-center gap-3 mt-6">
        ...
        <button
          onClick={() => onSave?.(questions.filter((q) => selected.has(q.id)))}
          className="px-4 py-2 rounded-xl border border-primary bg-primary text-white text-sm hover:bg-primary-hover transition-colors flex items-center gap-2"
        >
